...
class ForumParser {
    ...
    private fun sanitizePostHtml(msgNode: Element): String {
        val content = msgNode.clone()
        content.select("script,style,iframe,object,embed,form,input,textarea,button,video,audio,noscript").remove()
        content.select("[style]").removeAttr("style")
        content.select("[class]").removeAttr("class")

        content.select("table").forEach { table ->
            val cells = table.select("td,th").size
            val textLen = table.text().length
            if (cells > 200 || textLen > 4000) {
                table.replaceWith(Element("div").text("[表格内容过大，已省略]"))
            }
        }
