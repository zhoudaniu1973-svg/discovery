    ...
    private fun observeViewModel() {
        viewModel.posts.observe(this) { posts ->
            adapter.submitList(posts) {
                if (!hasAutoScrolled && posts.isNotEmpty()) {
        ...
        viewModel.needWebViewFetch.observe(this) { request ->
            if (request != null) {
                fetchViaWebView(request.url, request.append)
            }
        }
    }

    private fun fetchViaWebView(url: String, append: Boolean) {
        ...
        fetcher.fetch(url, object : WebViewFetcher.Callback {
            override fun onSuccess(html: String) {
                viewModel.handleWebViewResult(html, append = append)
            }
